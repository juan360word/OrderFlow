...
def upgrade() -> None:
    # Extensions first: the columns below are typed with them.
    #   citext   -> case-insensitive UNIQUE email
    #   pgcrypto -> gen_random_uuid() as a server-side default
    ...
    # Seed the two roles the application requires. Idempotent, so re-running on
...
def downgrade() -> None:
    ...
    # CREATE TYPE was implicit in create_table; DROP TYPE is not. Without this
    # the downgrade leaves an orphan type and the next upgrade fails with
    # "type reservation_status already exists".
